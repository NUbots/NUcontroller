...
namespace nusense {
    ...
    namespace convert {
        ...
        float velocity(int32_t velocity) {
            ...
            return utility::math::clamp(int32_t(-167), velocity, int32_t(167)) * 0.229f * 2 * M_PI / 60.0f;
        }
        ...
        int32_t velocity(float velocity) {
            ...
            // Default servo limits for velocity - minimum of all for X-Series, MX-106 and MX-64
            // X-Series has the minimum at 167
            return int32_t(utility::math::clamp(-167.0f, (velocity * 60.0f / (0.229f * 2 * M_PI)), 167.0f));
        }
